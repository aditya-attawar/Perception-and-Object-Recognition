...
def main():
    if len(sys.argv) < 8:
        print('Usage: dlt_image.py left right cb_cols cb_rows point_x point_y output')
        sys.exit(1)
    ...
    # Final output with the corresponding point of the given test point
    ...
    cv2.circle(right_image, (np.float32(final_point[0][0]), np.float32(final_point[1][0])), radius=2, thickness=3,
               color=(0, 255, 0))
    composite_image = cv2.hconcat([left_image, right_image])
    cv2.imwrite(output_filename, composite_image)
    # cv2.imshow("Homography Transformation", composite_image)
    # cv2.waitKey(0)
